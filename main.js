...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const circleFinal   = isMobileProc ? 80 : CIRCLE_FINAL; // smaller circle on mobile → more room for text
  ...
  function updateProcessSection() {
    ...
    if (prog <= 0.25) {
      ...
      const offset = isMobileProc
        ? (1 - p) * 80 + 55 * p   // mobile: 80px → 55px (more padding from smaller circle)
    ...
    // ── Phase D (0.70 → 1.0): PORTA + PATRIA slide out from circle, Card 2 slides up ─
      ...
      const offset = isMobileProc
        ? p * 25 + 55   // mobile: 55px → 80px (generous gap, stays in viewport)
