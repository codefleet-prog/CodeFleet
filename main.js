...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateHeroScroll() {
    if (!heroTicking) {
      requestAnimationFrame(() => {
        ...
        if (pinkCircle) {
          const BASE = 3000; // matches CSS width/height in px
  ...
  if (servicesScrollArea && servicePanels.length) {
    ...
    // On mobile, offset the sticky panel below the fixed navbar so the counter
    // bar snaps right under it (not hidden behind it).
    function syncPanelHeights() {
      ...
      if (stickyPanel) {
        ...
        stickyPanel.style.height = cachedVH + 'px';
      }
    }
    syncPanelHeights();
