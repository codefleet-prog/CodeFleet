  ...
  function updateHeroScroll() {
    if (!heroTicking) {
      requestAnimationFrame(() => {
        ...
        // Track BL blob's position — only needed once per frame, not per scroll event
        if (techBL) {
        ...
        document.documentElement.style.setProperty('--circle-size', (progress * maxSize) + 'px');
        ...
        if (parallaxBtn) {
          ...
          const btnParallax = Math.max(0, relScroll * 0.12);
