...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateHeroScroll() {
    ...
    // Track BL blob's position on screen so the fixed pink circle stays anchored to it
    if (techBL) {
    ...
    // Animation starts after 300px of scroll
    const animStart = 300;
    const animDistance = 800;
    const progress = Math.min(Math.max((scrollY - animStart) / animDistance, 0), 1);

    // Pink circle grows to cover the screen
    const maxSize = Math.max(window.innerWidth, window.innerHeight) * 2.8;
    const circleSize = progress * maxSize;
    document.documentElement.style.setProperty('--circle-size', circleSize + 'px');

    // Parallax effect for about-content to slowly move up quicker
    const aboutContent = document.querySelector('.about-content');
    if (aboutContent) {
      const parallax = scrollY * 0.25; // increased from 0.15
      aboutContent.style.transform = `translateY(-${parallax}px)`;
    }

    // Independent faster parallax for the brave button
    const pWrapper = document.querySelector('.parallax-btn-wrapper');
    const parallaxBtn = document.querySelector('.parallax-btn');
    if (pWrapper && parallaxBtn) {
      const rect = pWrapper.getBoundingClientRect();
      // Distance from center of screen
      const distFromCenter = rect.top - (window.innerHeight / 2);
      // Moves upward slightly faster as you scroll past it
      const btnParallax = distFromCenter * -0.2; 
      parallaxBtn.style.transform = `translateY(${btnParallax}px)`;
    }
  }
  ...
  menuToggle.addEventListener('click', () => {
    ...
    menuLabel.textContent = menuOpen ? 'BEZÁR' : 'MENÜ';
  });
  ...
  if (servicesScrollArea && servicePanels.length) {
    ...
    // Set the scroll area tall enough for all panels
    servicesScrollArea.style.height = `${numServices * 100}vh`;
    ...
    function updateServices() {
      ...
      if (servicesHeader && servicesMegaTitle) {
        const headerRect = servicesHeader.getBoundingClientRect();
        const windowH = window.innerHeight;
        const scrubProgress = Math.min(Math.max((windowH - headerRect.top) / (windowH * 0.7), 0), 1);
        const clipVal = 100 - (scrubProgress * 100);
        const transX = -1.5 + (scrubProgress * 1.5);
        servicesMegaTitle.style.webkitClipPath = `inset(0 ${clipVal}% 0 0)`;
        servicesMegaTitle.style.clipPath = `inset(0 ${clipVal}% 0 0)`;
        servicesMegaTitle.style.transform = `translateX(${transX}rem)`;
        if (servicesEyebrow) {
          servicesEyebrow.style.opacity = scrubProgress;
          servicesEyebrow.style.transform = `translateY(${10 - (scrubProgress * 10)}px)`;
        }
      }
      ...
      if (scrollInArea < 0) return;

      const maxOffset = (numServices - 1) * window.innerHeight;
      ...
      const newIndex = Math.min(
        Math.floor(scrollInArea / window.innerHeight),
        numServices - 1
      );

      if (newIndex !== lastServiceIndex && servicesNumInner && slotHeight > 0) {
        servicesNumInner.style.transform = `translateY(-${newIndex * slotHeight}px)`;
        lastServiceIndex = newIndex;
      }
    }

    window.addEventListener('scroll', updateServices, { passive: true });
    window.addEventListener('resize', () => { initSlotHeight(); updateServices(); });
  ...
  function setCircle(top, size, color, spin, opacity) {
    ...
    fixedCircle.style.opacity    = opacity;
    fixedCircle.style.width      = `${size}px`;
    fixedCircle.style.height     = `${size}px`;
    fixedCircle.style.top        = `${top}px`;
    fixedCircle.style.left       = `${window.innerWidth / 2}px`;
    fixedCircle.style.background = color;
    fixedCircle.style.transform  = `translate(-50%,-50%) rotate(${spin}deg)`;
  }
  ...
  function updateProcessSection() {
    ...
    if (prog <= 0.25) {
      ...
      const offset = (1 - p) * 125 + 75 * p; // from 125px → 75px
      const tOpacity = 1 - p;
      if (dorkoText) {
      ...
      setCircle(circleCenter, CIRCLE_FINAL, PURPLE, 0, 1);
    }
    // ── Phase B (0.25 → 0.50): Circle spins + exits downward, bg crossfades, Card 1 exits ─
      ...
      const spin = p * 360;
      const exitY = p * vh;
      setCircle(circleCenter + exitY, CIRCLE_FINAL, PURPLE, spin, 1 - p * 0.3);
      if (dorkoText) dorkoText.style.opacity = '0';
      if (padelText) padelText.style.opacity = '0';
      if (portaText) portaText.style.opacity = '0';
      if (patriaText) patriaText.style.opacity = '0';
    ...
    // ── Phase C (0.50 → 0.70): Pink circle enters from top, spinning ─
      ...
      const spin = (1 - p) * -360;
      const entryY = (1 - p) * -vh;
      setCircle(circleCenter + entryY, CIRCLE_FINAL, PINK, spin, 1);
      if (dorkoText) dorkoText.style.opacity = '0';
      if (padelText) padelText.style.opacity = '0';
      if (portaText) portaText.style.opacity = '0';
      if (patriaText) patriaText.style.opacity = '0';
    ...
    // ── Phase D (0.70 → 1.0): PORTA + PATRIA slide out from circle, Card 2 slides up ─
      ...
      const offset = p * 125 + 75; // from 75px → 200px
      ...
      setCircle(finalTop, CIRCLE_FINAL, PINK, 0, 1);
